...
async function runTests() {
  ...
  const adminLogin1 = apiDispatcher('auth/login', 'POST', { email: 'admin@iitkgp.ac.in', password: 'AdminMaster#2026' });
  console.assert(adminLogin1.ok === true, 'Admin login with AdminMaster#2026 should succeed');
  ...
  const adminLogin2 = apiDispatcher('auth/login', 'POST', { id: 'ADMIN01', password: 'Admin@IITKGP2026' });
  console.assert(adminLogin2.ok === true, 'Admin login with ADMIN01 and Admin@IITKGP2026 should succeed');
  ...
  const empLogin1 = apiDispatcher('auth/login', 'POST', { email: 'EMP01', password: 'EmpNorth#2026' });
  ...
  const empLogin2 = apiDispatcher('auth/login', 'POST', { email: 'emp01@ndli.iitkgp.ac.in', password: 'EmpNorth#2026' });
  ...
  const verifyPass = apiDispatcher('auth/verify-password', 'POST', { user_id: 'EMP01', password: 'EmpNorth#2026' });
  ...
  const blockedLogin = apiDispatcher('auth/login', 'POST', { email: 'EMP07', password: 'EmpSouth2#2026' });
  ...
  const adminFallback = apiDispatcher('auth/login', 'POST', { email: 'admin@iitkgp.ac.in', password: 'AdminMaster#2026' });
  console.assert(adminFallback.ok === true && adminFallback.data.user.role === 'ADMIN', 'Admin fallback login should succeed');
  const emp2Fallback = apiDispatcher('auth/login', 'POST', { email: 'EMP02', password: 'EmpCentral#2026' });
  ...
  const strBodyRes = apiDispatcher('auth/login', 'POST', JSON.stringify({ email: 'ADMIN01', password: 'AdminMaster#2026' }));
